...
If you need more control over the Drizzle client:

```typescript
import { createClient, createCaller, StoreDrizzle } from 'ff-ai/providers/drizzle/store';
import { ConversationStoreLayer } from 'ff-ai/providers/drizzle/conversation';
import { Layer } from 'effect';

const client = createClient(sql, {
  casing: 'snake_case',
  // Other Drizzle options...
});
...
Use different databases for different parts of your app:

```typescript
const mainSql = postgres(process.env.MAIN_DB_URL!);
const aiSql = postgres(process.env.AI_DB_URL!);

const mainStoreLayer = createDrizzleStoreLayer(mainSql);
const aiStoreLayer = createDrizzleStoreLayer(aiSql);

// Use aiStoreLayer for AI conversations
const aiProgram = Effect.provide(conversationLogic, aiStoreLayer);
```
...
```typescript
import { Effect } from 'effect';
import { StoreDrizzle } from 'ff-ai/providers/drizzle/store';
import { threads, messages } from 'ff-ai/providers/drizzle/schema';
...
const program = Effect.gen(function* () {
  ...
  // Custom query
  const result = yield* db.call((drizzle) =>
    drizzle
      .select()
      .from(threads)
      .where(eq(threads.resourceId, 'user-123'))
  );
