...
import type { ThreadIdentifier } from 'ff-ai/conversation/core/thread';

class InMemoryStore {
  private messages = new Map<string, ConversationMessage.Type[]>();
  ...
  ): Effect.Effect<ConversationMessage.Type[], StoreError> {
    return Effect.try({
      try: () => {
  ...
  saveMessages(
    params: ThreadIdentifier & { messages: ConversationMessage.Type[] }
